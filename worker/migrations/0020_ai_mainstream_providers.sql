-- Widen the provider constraint without resetting credentials, generations, leases or usage.
-- Execute as one D1 migration; existing provider-bound v1 ciphertext is copied unchanged.
CREATE TABLE ai_connection_next (
  id INTEGER PRIMARY KEY CHECK (id=1),
  provider TEXT NOT NULL DEFAULT 'zen' CHECK (provider IN ('zen','gemini','openai','anthropic','deepseek','groq','openrouter')),
