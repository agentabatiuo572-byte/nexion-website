...
import { dailyJob, isUtcDay, runDailyRollup } from './rollup';
...
app.use('/api/publish', requirePublishIdentity);
app.use('/api/publish/*', requirePublishIdentity);
...
app.post('/api/admin/rollup', requireAuth, async (c) => {
  ...
  if (!body?.date || !isUtcDay(body.date)) return c.json({ error: 'bad-date' }, 400);
  const result = await runDailyRollup(c.env.DB, body.date, { action: 'admin.rollup', target: body.date });
  return c.json({
    ok: true,
    status: result.rejectedEvents > 0 ? 'completed-with-rejections' : 'completed',
    ...result,
  });
});
...
// 静态产物兜底(T3;区域屏蔽中间件 T15 已插在一切之前)
  // 404 命中计数(CON03-③ 质量卡):服务端侧记,比页内埋点准；爬虫保留 raw 证据但不进真人质量卡
app.all('*', async (c) => {
  ...
  if (res.status === 404 && (c.req.header('accept') ?? '').includes('text/html')) {
    ...
    if (!notFoundLimiter.hit(ip)) {
      const payload = {
        t: 'e404',
        path: truncateUtf8(new URL(c.req.url).pathname, METRIC_TEXT_BYTES.path),
        bot: isBotUserAgent(c.req.header('user-agent') ?? '') ? 1 : 0,
      };
      const log = c.env.DB.prepare('INSERT INTO raw_events (ts, type, uid, payload) VALUES (?1, ?2, NULL, ?3)')
        .bind(Date.now(), 'e404', JSON.stringify(payload))
        .run()
        .then(() => {})
        .catch(() => {});
      try {
