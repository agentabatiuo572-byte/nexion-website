...
| D 产物不回归 | PASS | `node test-static.mjs` exit 0:`/api/health` 200 + **36/36 路由伺服透明**(⚠️ 2026-09-01 更正:这条证明的是「响应体 == 伺服目录里的文件」,两边同源,**证明不了产物没被改过** —— 那需要和一个独立基线比。伺服目录被改回 dist 由 gate-config-consistency 抓) + 未知路径 404(8788 自起自杀,回读 0 监听);`npm run gate:beacon` exit 0(内联埋点 gzip 1477B ≤2048,37 页每页恰 1 份);统计数字在产物页上:`dist/index.html` 含 `28,432`、`dist/vi/index.html` 含 `28.432`(vi 千分位点,与 fmtStat localeTag 一致)、`dist/zh/index.html` 含 `28,432` —— 渲染链未断 |
