...
- Astro 5 + Tailwind 4(`@tailwindcss/vite`,无 config)。**零框架 JS**:React 岛已全部移除(cobe 地球 → 自研洛伦兹 canvas)。交互引擎单文件 `src/scripts/fx.ts`:①洛伦兹粒子背景(鼠标推斥/倾斜) ②Lenis 平滑滚动 ③琥珀圆点光标 ④data-rv 进场 reveal ⑤UTC 时钟 ⑥设备 pinned 横向轨——全部 reduced-motion 降级。
