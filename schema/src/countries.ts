/* ISO 3166-1 alpha-2 全集(代码+中文名)——**单一真源**:控制台下拉与服务端 PUT 校验同 import。
   2026-08-31 第二路验收 P2:此前面板用本表、API 只校验 /^[A-Z]{2}$/,两套判据 → ZZ/XX 等非真实码能经 API 入名单;
   尤其 XX 是中间件「无法识别国家」的兜底值,入名单将误伤全部无地理信息访客(见 SENTINEL_COUNTRY)。
...
/** 中间件在拿不到国家信息时用的兜底值。🔴 它**不是**一个国家,禁止入屏蔽名单——
    入了等于把「所有无地理信息的访客」一并拦掉(含部分企业代理/隐私网络的正常用户)。 */
export const SENTINEL_COUNTRY = 'XX';

const CODES = new Set(ISO_COUNTRIES.map((c) => c.code));
/** 服务端校验单源:必须是真实 ISO 码,且不能是兜底哨兵 */
export const isRealCountryCode = (code: string): boolean => CODES.has(code) && code !== SENTINEL_COUNTRY;
